...
public:
    ...
    value_type *GetDataPtr(size_type pos) const {
        if (pos >= _max_size) return nullptr;
        node_type *ptr_node = GetNodePtr(pos);

        if (!ptr_node->is_use) return nullptr;
...
public:
    // constructor
    ListIterator(state_type *ptr_state = nullptr, size_type pos = 0) : _ptr_state(ptr_state), _pos(pos) {}
    ...
    ListIterator &operator++() {
        _pos = _ptr_state->GetNextPos(_pos);
        return *this;
    }
    ListIterator &operator--() {
        _pos = _ptr_state->GetPrevPos(_pos);
        return *this;
    }
