...
public:
    ...
    bool CheckConsist() const
    {
        ...
        for (size_t pos = GetRChildPos(_empty_head_pos); pos != _empty_head_pos && _empty_head_pos != NPOS; pos = GetRChildPos(pos)) {
    ...
    void InsertFixup(size_type new_pos) 
    {
        ...
        //         \                    /               |
        //          new[R]            parent[R]         |
        if (GetRChildPos(parent_pos) == new_pos && 
            GetLChildPos(grand_parent_pos) == parent_pos)
        {
    ...
    void EraseFixup(size_type parent_pos, size_type pos) 
    {
        ...
        if (GetColor(parent_pos)  == BLACK &&
            GetColor(brother_pos) == BLACK &&
            GetColor(blchild_pos) == BLACK &&
        ...
        if (GetRChildPos(parent_pos) == pos &&
            GetColor(blchild_pos) == BLACK &&
            GetColor(brchild_pos) == RED)
        {
            SetColor(brother_pos, RED);
    ...
    size_type GetLChildPos(size_type pos) const { if (pos == NPOS) return NPOS; return GetNodePtr(pos)->lchild_pos; }
    size_type GetRChildPos(size_type pos) const { if (pos == NPOS) return NPOS; return GetNodePtr(pos)->rchild_pos; }
...
public:
    // constructor
    RBTreeIterator(state_type * ptr_state = NULL, size_type pos = 0)
    ...
    RBTreeIterator & operator++() { _pos = _ptr_state->FindNextBigPos(_pos); return *this; }
    RBTreeIterator & operator--() { _pos = _ptr_state->FindNextSmallPos(_pos); return *this; } 
