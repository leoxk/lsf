...
LSF_TEST_CASE(test_sockaddr_v4_and_v6) {
    ...
    LSF_ASSERT(udp::SockAddr(Address(AF_INET, "192.168.1.1"), 32).ToString() == "192.168.1.1|32");
    LSF_ASSERT(udp::SockAddr(Address(AF_INET6, "::ff:192.168.1.1"), 32).ToString() == "::ff:c0a8:101|32");

    udp::SockAddr sockaddr(Address(AF_INET, "192.168.1.1"), 32);
...
LSF_TEST_CASE(test_sock_op_v4) {
    ...
    // test bind
    LSF_ASSERT(socket.Bind(udp::SockAddr(Address(), 18888)));
    LSF_ASSERT(socket.LocalSockAddr() == udp::SockAddr(Address(), 18888));
    LSF_ASSERT(socket.RemoteSockAddr() == udp::SockAddr::Any());

    // test connect
    LSF_ASSERT(socket.Connect(udp::SockAddr(Address::Any(), 19999)));
    LSF_ASSERT(socket.LocalSockAddr() == udp::SockAddr(Address::Loopback(), 18888));
    LSF_ASSERT(socket.RemoteSockAddr() == udp::SockAddr(Address::Loopback(), 19999));
}
