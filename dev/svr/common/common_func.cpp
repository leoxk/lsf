...
bool GetSingleMessageFromStream(std::string const & buffer, size_t & pos, std::string & message)
{
    ...
    {
        LSF_LOG_ERR("incomplete message, buffer_length=%u, header_length=%u, %s", length, header.length, StringExt::BinToHexString(begin, length).c_str());
        return false;
    }
